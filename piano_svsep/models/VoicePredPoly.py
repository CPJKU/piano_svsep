...
class GraclusPooling(torch.nn.Module):
    def __init__(self, hidden_channels, dropout=0.5):
        super().__init__()
        self.edge_weights_mlp = nn.Sequential(
    ...
    def forward(self, x, edge_index, batch):
        z = torch.cat((x[edge_index[0]], x[edge_index[1]]), dim=-1)
        edge_score = self.edge_weights_mlp(z).squeeze()
        cluster = gnn.graclus(edge_index, edge_score, num_nodes=x.size(0))
        new_x = scatter(x, cluster, dim=0, reduce='mean')
...
class PostProcessPoolingGraclus(nn.Module):
    def __init__(self, threshold=0.5):
        super().__init__()
        self.threshold = threshold

    def forward(self, edge_index, edge_probs, chord_edge_index, batch, chord_edge_score, num_nodes):
        x = torch.arange(num_nodes, device=edge_index.device)
        edge_score = (chord_edge_score > self.threshold).float()
        cluster = gnn.graclus(chord_edge_index, edge_score, num_nodes=x.size(0))
        new_x = scatter(x, cluster, dim=0, reduce='mean')
        new_edge_index, new_edge_probs = coalesce(cluster[edge_index], edge_probs, num_nodes=new_x.size(0))
        ...
        unpool_info = UnpoolInfo(edge_index=edge_index, cluster=cluster,
                                 batch=batch, new_edge_score=edge_score)
        reduced_num_nodes = new_x.size(0)
